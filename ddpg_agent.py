...
#         mean_r = np.mean(reward_np)
#         std_r = np.std(reward_np)+ 1.0e-10
# #         print("mean ",mean_r)
# #         print("std ",std_r)
#         reward_np = (reward_np - mean_r)/std_r
#         reward = reward_np.tolist()
        i = 0
...
#             print(i, " r " ,r)
        ...
        if len(self.memory) > BATCH_SIZE and (t % UPDATE_EVERY_T == 0):
            for i in range(0,NUM_UPDATES):
                experiences = self.memory.sample()
                self.learn(experiences, GAMMA)
                if(self.num_upd_g < 50):
                    self.num_upd_g = self.num_upd_g + 1
